...
export default function Procurement(){
  // sample dataset of 24 tenders (for pagination demo)
  const base = [
    ...
    <main className="dashboard">
      <div className="container">
        ...
        <div className="tabs card">
          <div className="tab active">Active Tenders <span className="pill">24</span></div>
          <div className="tab">Draft <span className="pill">6</span></div>
          <div className="tab">Under Evaluation <span className="pill">10</span></div>
          <div className="tab">Awarded <span className="pill">32</span></div>
          <div className="tab">Archived <span className="pill">78</span></div>
        </div>
          ...
          <div className="table-footer">
            ...
            <div className="pagination">
              ...
              {Array.from({length:pageCount}).map((_,p)=>(
                <button key={p} className={`page-num ${currentPage===p+1? 'active':''}`} onClick={()=>setCurrentPage(p+1)}>{p+1}</button>
              ))}
              <button className="page-btn" onClick={()=>setCurrentPage(p=>Math.min(pageCount,p+1))} disabled={currentPage===pageCount}>Next</button>
            </div>
          </div>
        </section>
      </div>
